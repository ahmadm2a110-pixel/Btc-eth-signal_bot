...
async def send_ntfy(
    ...
    try:
        ...
        logger.info(
            f"ntfy notification sent successfully | "
            f"status={response.status_code}"
        )
...
def analyze_5m(
    ...
    if close > ema20:
        ...
        short_score += 1
        short_reasons.append("قیمت پایین EMA20")

    if ema20 > ema50:

        long_score += 1
        long_reasons.append("EMA20 بالاتر از EMA50")

    elif ema20 < ema50:

        short_score += 1
        short_reasons.append("EMA20 پایین‌تر از EMA50")
...
def format_signal(signal):
    ...
    text += (
        "\n⚠️ این سیگنال صرفاً تحلیل تکنیکال است."
    )
...
async def scan_symbol(symbol):
    ...
    try:
        ...
        df_5m, df_15m, df_1h, df_4h = await asyncio.gather(
            fetch_data(symbol, TIMEFRAME_5M),
            fetch_data(symbol, TIMEFRAME_15M),
            fetch_data(symbol, TIMEFRAME_1H),
            fetch_data(symbol, TIMEFRAME_4H),
        )
...
async def scanner():
    ...
    logger.info("========================================")
    logger.info("CRYPTO SCALPING BOT STARTING")
    logger.info("========================================")
