...
def candle_key(symbol, candle_time):
    return f"{symbol}:{int(candle_time)}"
...
def detect_structure(df):
    ...
    ema20 = safe_float(recent["ema20"].iloc[-1])
    ema50 = safe_float(recent["ema50"].iloc[-1])
...
def analyze_direction(h1, h4, direction):
    ...
    if direction == "LONG":
        if h4 == "BULLISH":
            ...
            score += 1
            reasons.append("H4 trend mildly bullish")
        else:
            warnings.append("H4 trend not aligned")
        ...
        if h4 == "BEARISH":
            ...
            score += 1
            reasons.append("H4 trend mildly bearish")
        else:
            warnings.append("H4 trend not aligned")
    ...
    if direction == "LONG":
        if ema20 > ema50:
            score += 1
            reasons.append("H1 EMA20 above EMA50")

        if ema50 > ema200:
            score += 1
            reasons.append("H1 EMA50 above EMA200")

    else:
        if ema20 < ema50:
            score += 1
            reasons.append("H1 EMA20 below EMA50")

        if ema50 < ema200:
            score += 1
            reasons.append("H1 EMA50 below EMA200")
    ...
    if direction == "LONG":
        if structure == "BULL_BREAKOUT":
            ...
            score -= 2
            warnings.append("Bearish breakdown")

    else:
        if structure == "BEAR_BREAKDOWN":
            score += 2
            reasons.append("Bearish structure breakdown")
        elif structure == "BEARISH":
            score += 1
            reasons.append("Bearish market structure")
        elif structure == "BULL_BREAKOUT":
            score -= 2
            warnings.append("Bullish breakout")
    ...
    if candle_confirmation(row, direction):
        score += 1
    ...
    if detect_trendline_break(h1, direction):
        score += 1
        reasons.append("Price action supports direction")
    ...
    if distance_atr <= MAX_ENTRY_DISTANCE_ATR:
        ...
        warnings.append(
            f"Price extended {distance_atr:.2f} ATR from EMA20"
        )
    ...
    if direction == "LONG":
        if resistance > 0:
            if price > resistance:
                reasons.append("Above recent resistance")
            elif abs(price - resistance) <= atr * RETEST_TOLERANCE_ATR:
                reasons.append("Near resistance retest")

    else:
        if support > 0:
            if price < support:
                reasons.append("Below recent support")
            elif abs(price - support) <= atr * RETEST_TOLERANCE_ATR:
                reasons.append("Near support retest")
    ...
    # Hard invalidation
    if direction == "LONG":
        if h4 == "BEARISH" and structure == "BEAR_BREAKDOWN":
            grade = "NO_TRADE"

    else:
        if h4 == "BULLISH" and structure == "BULL_BREAKOUT":
            grade = "NO_TRADE"
...
async def send_ntfy(title, message, priority="default", tags=None):
    headers = {
        "Title": title,
        "Priority": priority,
    }
...
def format_setup(symbol, h4_context, result, trade_map):
    direction = result["direction"]
    ...
    return (
        f"{icon} {symbol} — {direction} SETUP\n"
        f"\n"
        f"Grade: {result['grade']}\n"
        f"Score: {result['score']}/10+\n"
        f"H4: {h4_context}\n"
        f"H1 Structure: {result['structure']}\n"
        f"\n"
        f"Price: {fmt_price(result['price'])}\n"
        f"RSI: {result['rsi']:.1f}\n"
        f"ADX: {result['adx']:.1f}\n"
        f"ATR: {fmt_price(result['atr'])}\n"
        f"EMA20 distance: {result['distance_atr']:.2f} ATR\n"
        f"\n"
        f"CONFIRMATIONS\n"
        f"{reasons}\n"
        f"\n"
        ...
        f"\n"
        f"Action: WAIT FOR CONFIRMATION / RETEST"
    )
...
def get_setup_id(symbol, result):
    return (
        f"{symbol}:"
        f"{result['direction']}:"
        f"{result['structure']}"
...
def should_alert(setup_id, result):
    current_time = now_utc()

    previous = last_alert.get(setup_id)
...
def save_alert(setup_id, result):
    last_alert[setup_id] = {
        "score": result["score"],
        "grade": result["grade"],
        "time": now_utc(),
...
def monitor_previous_setup(symbol, h1, h4_context):
    state = setup_state.get(symbol)
...
def update_setup_state(symbol, result):
    setup_state[symbol] = {
        "direction": result["direction"],
        "score": result["score"],
        "grade": result["grade"],
...
async def analyze_symbol(symbol):
    ...
    # --------------------------------------------------------
    # Existing setup monitor
    # --------------------------------------------------------

    previous_state = setup_state.get(symbol)
        ...
        if monitor:
            ...
            if event == "INVALIDATED":
                await send_ntfy(
                    ...
                    (
                        f"{symbol} {previous_state['direction']} setup "
                        f"has been invalidated.\n\n"
                        f"H4: {h4_context}\n"
                ...
                setup_state.pop(symbol, None)

            elif event == "STRENGTHENED":
                trade_map = build_trade_map(result)
    ...
    if best["grade"] in {
        ...
        trade_map = build_trade_map(best)
        ...
        if should_alert(
            ...
            await send_ntfy(
                f"👀 {symbol} WATCH",
                (
                    f"{symbol} {best['direction']}\n\n"
                    f"Score: {best['score']}\n"
                    f"H4: {h4_context}\n"
                    f"H1: {best['structure']}\n"
                    f"RSI: {best['rsi']:.1f}\n"
                    f"ADX: {best['adx']:.1f}\n\n"
                    f"Action: WAIT FOR CONFIRMATION"
                ),
                priority="default",
                tags="eyes",
            )
